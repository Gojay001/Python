...
class JDCommentsCrawler:
    ...
    def concatLinkParam(self):
        self.locationUrl = self.locationLink + '?' + \
            self.paramDict2Str(self.paramValue) + 'isShadowSku=0&fold=1&page=0'
    ...
    def save_csv(self, df, p):
        # 保存文件
        df.to_csv(path_or_buf='file1/jdmish_%d.csv' % p, encoding='gbk')
...
def jdComment():
    # 设置关键变量
    page = 2000  # 页数
    productId = 7545792  # 商品ID
    callback = 'fetchJSON_comment98vv31430'  # 回调函数
